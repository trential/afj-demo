import { getUnqualifiedCredentialDefinitionId, type RegisterCredentialDefinitionReturnStateFinished } from '@aries-framework/anoncreds'
...
export class Faber extends BaseAgent {
  ...
  private getIndyCredDefId(credentialDefinitionId: string) {
    const splitCredDefId = credentialDefinitionId.split('/');
    const issuerId = splitCredDefId[0];
    return getUnqualifiedCredentialDefinitionId(issuerId.split(':')[4], splitCredDefId[4], splitCredDefId[5])
  }
    ...
    await this.agent.credentials.offerCredential({
      ...
      credentialFormats: {
        indy: {
          ...
          credentialDefinitionId: this.getIndyCredDefId(credentialDefinition.credentialDefinitionId),
        },
      },
    })
    this.ui.updateBottomBar(
        ...
        restrictions: [
          {
            cred_def_id: this.credentialDefinition?.credentialDefinitionId ?
              this.getIndyCredDefId(this.credentialDefinition?.credentialDefinitionId) : undefined,
          },
        ],
      },
    }
